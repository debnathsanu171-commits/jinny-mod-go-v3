...
import { getRls, RLS_GUC_SQL, rlsGucParams } from "./rls.server";
  ...
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    return toSql(async <T>(text: string, params: unknown[]) => {
      ...
      try {
        ...
        await client.query(RLS_GUC_SQL, rlsGucParams(ctx));
...
async function createPgliteSql(): Promise<Sql> {
  ...
  return toSql(async <T>(text: string, params: unknown[]) => {
    ...
    return pg.transaction(async (tx) => {
      await tx.query(RLS_GUC_SQL, rlsGucParams(ctx));
...
export function ensureDbReady(): Promise<void> {
  ...
  return getSql().then(async () => {
    try {
      const { seedPlatformAdmin } = await import("./seed-admin.server");
      await seedPlatformAdmin();
    } catch (err) {
      console.error("[db] Seed admin error:", err);
    }
  });
}
